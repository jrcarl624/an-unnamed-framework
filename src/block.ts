...
import { Vector, Block,BlockPermutation, Location, BlockLocation, world as World,MinecraftDimensionTypes } from "mojang-minecraft";
...
const error = (code: number | string, line: number, message?: string,) => {
    switch (code) {
        case 1: message = "Each blocks dimension must be the same";
...
type BlockPermutationChunkData = Record<number,Record<number,Record<number,BlockPermutation>>>


let BlockChunkDataExample = { "1": {"1":{"1":World.getDimension("overworld").getBlock(new BlockLocation (1,1,1))}} }
...
const BlockPermutationChunk = class {
    ...
    constructor(block: Block, dBlock: Block, data?) {
        ...
        if (data) {
            ...
            for (let i of block.location.blocksBetween(dBlock.location)) data[
                i.x -
                Math.sqrt(
                    Math.pow(
                        dBlock.location.x - block.location.x,
                        2
                    )
                )
            ][
                ...
                Math.sqrt(
                    Math.pow(
                        dBlock.location.y - block.location.y,
                        2
                    )
                )
            ][
                ...
                Math.sqrt(
                    Math.pow(
                        dBlock.location.z - block.location.z,
                        2
                    )
                )
